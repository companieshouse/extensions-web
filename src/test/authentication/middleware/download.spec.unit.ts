// mock the apiclient
const mockGetFullRequest: jest.Mock = jest.fn(
  (companyId: any, token: any, requestId: any) => extensionsRequest
);
...
// mock the session
const mockSession: Session = jest.createMockFromModule(
  "../../../session/session"
);
...
describe("Download authorisation tests", () => {
  ...
  it("Should return error page if not authorised", async () => {
    mockSession.userProfile = jest
      .fn()
      .mockImplementation(() => userProfileIncorrectPermissions);
    mockSession.accessToken = jest.fn().mockImplementation(() => ACCESS_TOKEN);

    await authenticateForDownload(req as any, res, mockNextFunc);

    expect(mockGetFullRequest).toHaveBeenCalledWith(
      req.params[REQ_PARAM_COMPANY_ID],
      ACCESS_TOKEN,
      req.params[REQ_PARAM_REQUEST_ID]
    );
  ...
  it("Should call the next function in the middleware chain if user is in correct role", async () => {
    mockSession.userProfile = jest
      .fn()
      .mockImplementation(() => userProfileCorrectPermissions);
  ...
  it("Should call the next function in the middleware chain if user created the extension request", async () => {
    mockSession.userProfile = jest
      .fn()
      .mockImplementation(() => userProfileCorrectId);
