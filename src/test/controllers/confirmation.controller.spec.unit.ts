...
const AUTH_CODE_JOURNEY_TEXT: string =
  "You should receive the authentication code at the registered office within 5 working days.";
const ERROR_PAGE: string = "Sorry, there is a problem with the service";

const mockCacheService = (<unknown>loadSession) as jest.Mock<
  typeof loadSession
>;
const mockCallProcessorApi = (<unknown>callProcessorApi) as jest.Mock<
  typeof callProcessorApi
>;
...
>;
const mockUpdateExtensionSessionValue = (<unknown>(
  updateExtensionSessionValue
)) as jest.Mock<typeof updateExtensionSessionValue>;
...
beforeEach(() => {
  ...
  mockGetCompanyInContext.prototype.constructor.mockImplementation(
    () => "00006400"
  );
  mockUpdateExtensionSessionValue.prototype.constructor.mockImplementationOnce(
    () => Promise.resolve()
  );
});
...
describe("confirmation controller", () => {
  it("should render the confirmation page", async () => {
    ...
    mockCacheService.prototype.constructor.mockResolvedValueOnce(
      dummySession(COMPANY_NUMBER, EMAIL)
    );
  ...
  it("should render the confirmation page with missing auth code text if that is a reason", async () => {
    mockCacheService.mockClear();
    mockCacheService.prototype.constructor.mockResolvedValueOnce(
      dummySessionWithToken(COMPANY_NUMBER, EMAIL)
    );
  ...
  it("should set already submitted to true in routine call", async () => {
    ...
    expect(mockUpdateExtensionSessionValue).toHaveBeenCalledWith(
      session,
      keys.ALREADY_SUBMITTED,
      true
    );
    expect(resp.status).toEqual(200);
  });
  ...
  it("should set already submitted to false if error", async () => {
    ...
    const session: Session = dummySessionWithToken(COMPANY_NUMBER, EMAIL);
    mockCacheService.prototype.constructor.mockResolvedValueOnce(session);
    mockCallProcessorApi.prototype.constructor.mockImplementationOnce(() =>
    ...
    expect(mockCallProcessorApi).toHaveBeenCalled();
    expect(mockUpdateExtensionSessionValue).toHaveBeenCalledTimes(2);
    expect(mockUpdateExtensionSessionValue).toHaveBeenCalledWith(
      session,
      keys.ALREADY_SUBMITTED,
      true
