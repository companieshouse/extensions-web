...
const mockCacheService = (<unknown>loadSession) as jest.Mock<
  typeof loadSession
>;
const mockAddAttachment = (<unknown>addAttachmentToReason) as jest.Mock<
  typeof addAttachmentToReason
>;
const mockRequest = (<unknown>getRequest) as jest.Mock<typeof getRequest>;
const mockCompanyContext = (<unknown>getCompanyInContext) as jest.Mock<
...
beforeEach(() => {
  ...
  mockCompanyContext.prototype.constructor.mockImplementationOnce(
    () => "00006400"
  );
...
describe("document upload url tests", () => {
  ...
  it("should render error message when file is too big", async () => {
    ...
    const response = await request(app)
      ...
      .attach(
        "file-upload",
        path.join(__dirname + "/../client/files/text_large.txt")
      );
  ...
  it("should show error message if continue pressed with no docs added", async () => {
    ...
    expect(res.text).toContain(
      "You must add a document or click &quot;Continue without " +
        "adding documents&quot;"
    );
  });
  ...
  it("AJAX - should render error message when file is too big", async () => {
    ...
    expect(responseObj.divs[0].divHtml).toContain(
      EXPECTED_MAX_FILE_SIZE_MESSAGE
    );

    expect(responseObj.divs[1].divHtml).toContain("govuk-file-upload");
    expect(responseObj.divs[1].divHtml).toContain(
      EXPECTED_MAX_FILE_SIZE_MESSAGE
    );
  ...
  it("AJAX - should return divs html when file uploaded successfully", async () => {
    ...
    expect(responseObj.divs[0].divHtml).toContain(
      "documentID=2ef57740-200d-4cb0-b814-c3149efcf87b"
    );
