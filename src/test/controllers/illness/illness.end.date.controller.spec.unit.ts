...
const ILLNESS_END_BEFORE_START_DATE: string =
...
beforeEach(() => {
  ...
  mockGetCurrentReason.prototype.constructor.mockImplementation(
    (): ReasonWeb => {
      return {
        id: "reason1",
        start_on: "2018-12-12",
...
describe("illness end date validation tests", () => {
  ...
  it("should show error if end date day is missing", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        "illness-end-day": "",
        "illness-end-month": "02",
        "illness-end-year": "2016",
      });
    expect(res.status).toEqual(200);
    expect(res.text).toContain(DAY_MISSING);
    expect(res.text).not.toContain(MONTH_MISSING);
    expect(res.text).not.toContain(YEAR_MISSING);
    expect(mockGetCurrentReason).toHaveBeenCalledTimes(1);
  });
  ...
  it("should show error message if date is in the future", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        "illness-end-day": "11",
        "illness-end-month": "05",
        "illness-end-year": "9999",
      });
    expect(res.status).toEqual(200);
  ...
  it("should show error message if end date precedes start date", async () => {
    ...
    expect(mockGetCurrentReason).toHaveBeenCalledTimes(2);
  });
});
