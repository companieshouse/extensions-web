...
const mockGetCurrentReasonFull = (<unknown>getCurrentReasonFull) as jest.Mock<
  typeof getCurrentReasonFull
>;
const mockLoggerError = (<unknown>logger.error) as jest.Mock<
  typeof logger.error
>;
...
const mockRenderFunc = jest
  .fn()
  .mockImplementation((view, options?, callback?: (err, html) => void) => {
    if (callback) {
...
describe("ajax upload responder tests", () => {
  ...
  it("should return error page redirect if exception occurs in success", async () => {
    ...
    res.render = jest
      ...
      .mockImplementationOnce(
        (view, options?, callback?: (err, html) => void) => {
          throw err;
        }
      );
  ...
  it("should call render error divs on user error", async () => {
    ...
    expect(mockRenderFunc.mock.calls[1][0]).toContain(UPLOAD_FILE_PICKER);
    expect(mockRenderFunc.mock.calls[1][1].documentUploadErr).toEqual(
      errorData
    );
    expect(mockSendFunc).toHaveBeenCalledWith({
      divs: [
