...
const mockUpdateReason = (<unknown>updateReason) as jest.Mock<
  typeof updateReason
>;
const mockDeleteReason = (<unknown>(
  removeExtensionReasonFromRequest
...
describe("reason service tests", () => {
  it("should call the apiClient to update a reason", async () => {
    await reasonService.updateReason(mockUtils.fullDummySession(), {
      illPerson: "joe",
    });

    expect(mockUpdateReason).toHaveBeenCalledWith(
  ...
  it("should delete a reason", async () => {
    ...
    expect(mockUpdateReason).not.toHaveBeenCalled();
    expect(mockDeleteReason).toHaveBeenCalledWith(
      session.extensionSession().extension_requests[0],
      "KGGGUYUYJHHVK1234"
    );
  });
});
...
it("should return a full reason for the current reason", async () => {
  ...
  expect(reason.attachments[2].id).toEqual(
    "444441-50b1-4930-a567-3ec83a668026"
  );
