import { unmarshalSignInInfo, unmarshalAccessToken, unmarshalUserProfile } from "../../session/store/util";
import { IAccessToken, IUserProfile, ISignInInfo, IMap } from "../../session/types";
...
describe("Session util tests", () => {
  ...
  describe("unmarshal access token tests", () => {
    ...
    it("should unmarshal an access token from session data", () => {
      ...
      expect(unmarshalledAccessToken.expiresIn).toEqual(parseInt(DUMMY_TOKEN.expires_in));
  ...
  describe("unmarshal signin info tests", () => {
    ...
    it("should return a ISigninInfo object if correct data in session", () => {
      ...
      expect((<unknown>unmarshalledSigninInfo.accessToken as IAccessToken).token).toEqual(DUMMY_TOKEN.access_token);
      expect((<unknown>unmarshalledSigninInfo.userProfile as IUserProfile).email).toEqual(DUMMY_PROFILE.email);
    });
